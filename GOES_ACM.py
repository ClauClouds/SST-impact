...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 10 22:07:46 2021
...
if case_study == 1:
    date = '23 Jan LT' 
    flist = GOES.locate_files(path + 'ACM/', 'OR_ABI-L2-ACMF*.nc','20200123-040000', '20200124-040400',
    ...
    date = '29-30 Jan LT' 
    flist = GOES.locate_files(path + 'ACM/', 'OR_ABI-L2-ACMF*.nc','20200129-040000', '20200131-040400',
                          use_parameter='scan_start_time')
    Merian_track = xr.open_dataset("https://observations.ipsl.fr/thredds/dodsC/EUREC4A/PRODUCTS/TRACKS/EUREC4A_tracks_MS-Merian_v1.0.nc")
    ...
    date = '2-3 Feb LT' 
    flist = GOES.locate_files(path + 'ACM/', 'OR_ABI-L2-ACMF*.nc','20200202-040000', '20200204-040400',
                          use_parameter='scan_start_time')
   # Merian_track = xr.open_dataset("https://observations.ipsl.fr/thredds/dodsC/EUREC4A/PRODUCTS/TRACKS/EUREC4A_tracks_MS-Merian_v1.0.nc")
...
new_filename = date + '_SST_MUR_regridded.nc'
print ('saving to ', new_filename)
SST.to_netcdf(path = path + new_filename)
SST.close()
print ('finished saving ' + new_filename)
...
new_filename = date + '_ACMall_GOES.nc'
print ('saving to ', new_filename)
...
for k in np.arange(180,288): #(0,flist.__len__()):
    fig = plt.figure(figsize=(14,8), dpi=300)
    plt.pcolormesh(BCMall.lon.data, BCMall.lat.data, BCMall[:,:,k].data, cmap = mycmap,
                 alpha=0.3)
    #plot SST
    ...
    plt.plot(Merian_track_int.lon[k*10], Merian_track_int.lat[k*10], 'ko')
    
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
    plt.title(timeBA[k].strftime('%d %b %H:%M') + ' LT (UTC-4): ' + 
              'GOES clear sky mask and SST MUR for 2 Feb',
...
plt.contourf(sst_mur30.lon.data, sst_mur30.lat.data, sst_mur30.sst_MUR.data,
             cmap = 'viridis', levels = np.arange(26.2, 28.1, 0.3))
plt.colorbar()
cs = plt.contour(sst_mur30.lon.data, sst_mur30.lat.data, sst_mur30.sst_MUR.data, 
               levels = [27.4, 27.7], colors = 'k')
cs.clabel(fontsize=12, inline=1, fmt='%.1f', colors='k')
plt.plot(Merian_track_int.lon, Merian_track_int.lat, color = 'r')
plt.plot(Merian_track_int.lon[0], Merian_track_int.lat[0], 'ro')
for i in range(60,2880,60):
    plt.plot(Merian_track_int.lon[i], Merian_track_int.lat[i], 'mx')
plt.plot(Merian_track_int.lon[1440], Merian_track_int.lat[1440], 'ro')
plt.xlabel('Longitude', fontsize = 16)
plt.ylabel('Latitude', fontsize = 16)
plt.title('SST MUR for 30 Jan and Merian track from 29/01 00 to 31/01 00 in LT')


plt.figure(dpi=300, figsize=(10,5))
plt.contourf(sst_mur29.lon.data, sst_mur29.lat.data, sst_mur29.sst_MUR.data,
             cmap = 'viridis', levels = np.arange(26.2, 28.1, 0.3))
plt.colorbar()
cs = plt.contour(sst_mur29.lon.data, sst_mur29.lat.data, sst_mur29.sst_MUR.data, 
               levels = [27.4, 27.7], colors = 'k')
cs.clabel(fontsize=12, inline=1, fmt='%.1f', colors='k')
plt.plot(Merian_track_int.lon, Merian_track_int.lat, color = 'r')
plt.plot(Merian_track_int.lon[0], Merian_track_int.lat[0], 'ro')
...
plt.xlabel('Longitude', fontsize = 16)
plt.ylabel('Latitude', fontsize = 16)
plt.title('SST MUR for 29 Jan and Merian track from 29/01 00 to 31/01 00 in LT')
...
index.values = np.ones(BCMall.shape, dtype=bool)

index.values[:,:, 0] = ~np.isnan(SST)
BCMall_wo_land = BCMall
BCMall_wo_land = BCMall.where(index, other = np.nan)
